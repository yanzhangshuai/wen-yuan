...
4. 覆盖率硬性门禁：每次执行单元测试与 coverage 校验时，必须同时满足 Statements >= 90%、Branches >= 85%、Functions >= 90%、Lines >= 90%；任一未达标即任务未完成，不得交付（高风险模块建议 >= 95%）。
...
- 覆盖率四项门禁（Statements >= 90%、Branches >= 85%、Functions >= 90%、Lines >= 90%）是否全部满足，并附有可复核证据？
