...
> 适用范围：知识库管理台中“姓氏词库 / 泛化称谓 / NER 词典规则 / Prompt 提取规则”的批量删除、启用、停用、设置书籍类型类操作。

## Scenario: Admin Knowledge Batch Operations

### 1. Scope / Trigger

- Trigger: 新增或调整 `src/app/api/admin/knowledge/*/batch/route.ts`、`src/lib/services/*` 中的 `batch*Action`、`src/server/modules/knowledge/*` 中的 `batchDelete*` / `batchToggle*` / `batchChangeBookType*`。
