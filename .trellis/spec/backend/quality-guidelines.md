...
- 覆盖率硬性门禁：每次执行单元测试与 coverage 校验时，必须同时满足 Statements >= 90%、Branches >= 85%、Functions >= 90%、Lines >= 90%；任一未达标即任务未完成，不得交付（高风险模块建议 >= 95%）。
...
- 测试注释是否足够支持复现与排障。
- 覆盖率证据是否可复核，且四项门禁（Statements >= 90%、Branches >= 85%、Functions >= 90%、Lines >= 90%）是否全部满足。
