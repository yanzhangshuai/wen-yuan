...
11. `T011` 书库首页基础版
12. `T012` 导入流程 Step 1-2
13. `T013` 导入流程 Step 3
14. `T014` 导入流程 Step 4
15. `T015` 全书解析编排服务
16. `T016` 解析状态查询与书库联动
17. `T017` ~ `T024` 图谱、原文、审核闭环
18. `T025` ~ `T032` 合并、手动管理、重解析
19. `T033` ~ `T037` 视觉、性能、Neo4j、格式扩展
20. `T038` ~ `T040` 契约、测试、全局体验补强

---

## 八、首批可以直接开写的 8 张票

如果现在立刻开始进入代码阶段，建议从这 8 张开始：
...
这 8 张票完成后，系统就拥有“管理员可登录 + 后台可守卫 + 模型可配置”的真实开发底座。
