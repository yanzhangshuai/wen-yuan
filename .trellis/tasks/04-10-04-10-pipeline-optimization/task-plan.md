...
// ──────────────── 1. 泛化称谓词库 ────────────────────
// 安全级：任何书籍类型下均不应作为独立人物（代词、泛泛称呼）
export const SAFETY_GENERIC_TITLES: ReadonlySet<string> = new Set([...]);

// 默认级：多数书籍类型下为泛称，但特定书籍类型可豁免（如武侠中"掌门"）
...
- 书籍类型预设使用 overlay 模式（基于默认配置覆盖差异项），新增书籍类型只需要加一个预设对象
...
| 泛化称谓示例截取前 30 个 | 按当前书籍类型配置动态生成（排除豁免词） |
| system prompt 过于简短 | 增加角色框架：{对应书籍类型专家} + {已知问题提醒} + {输出质量要求} |
...
user:
  ...
  ## 规则 (from ENTITY_EXTRACTION_RULES + 书籍类型特化补充)
...
- **Few-shot 动态示例**：根据书籍类型自动注入 2-3 个该书籍类型的典型识别案例
...
处理：
  ...
  2. 对比 GENRE_PRESETS 中各书籍类型的特征分布
  3. 匹配最接近的书籍类型（可同时匹配多个，取 top 2）
  4. 若置信度 < 0.7 → 提示用户手动选择
输出：推荐书籍类型 + 置信度
```
...
| 扩展 GENRE_PRESETS（7 种书籍类型） | A.1 | 小 | A.1 创建 |
...
| 合并阈值动态调整 | B.4 | 小 | 无 |
| 水浒传实测验证 | — | 大 | E（多书籍类型 goldset） |

### 里程碑 4：准确率 0.95+（预计 3-5 天）

| 任务 | 对应 Phase | 工作量 | 依赖 |
|------|-----------|--------|------|
| 多书籍类型 goldset 扩展 | E.3 | 大 | E |
| 书籍类型自动检测 | H | 中 | A.1 |
...
| 书籍类型预设新增无法验证 | 新增书籍类型效果未知 | 先在儒林外史验证框架，再扩展到其他小说 |
