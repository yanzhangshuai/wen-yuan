...
- `trellis --version` and `.trellis/.version` are both `0.4.0`.
- `trellis update --migrate` returns: "Already up to date!"
- Workflow smoke tests passed:
  - `python3 ./.trellis/scripts/get_context.py --mode packages`
  - `python3 ./.trellis/scripts/task.py list`
- Legacy split commands are removed from active command directories, unified files are present:
  - `.agents/skills/before-dev/SKILL.md`, `.agents/skills/check/SKILL.md`
  - `.claude/commands/trellis/before-dev.md`, `.claude/commands/trellis/check.md`
