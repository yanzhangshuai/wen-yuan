...
interface BookReviewSidebarProps {
  books        : BookLibraryListItem[];
  currentBookId: string;
}

export function BookReviewSidebar({
  books,
  currentBookId
}: BookReviewSidebarProps) {
  const [collapsed, setCollapsed] = useState(false);
  const currentBook = books.find(book => book.id === currentBookId);

  if (collapsed) {
    return (
      <aside className="book-review-sidebar w-12 shrink-0">
        <div className="sticky top-20 flex flex-col items-center gap-2">
          ...
          <Link
            href={`/admin/review/${currentBookId}`}
  ...
  return (
    <aside className="book-review-sidebar w-44 shrink-0">
