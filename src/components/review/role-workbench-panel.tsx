...
/**
 * =============================================================================
 * 文件定位（角色资料工作台主面板 Client Component）
 * -----------------------------------------------------------------------------
 * 文件路径：`src/components/review/role-workbench-panel.tsx`
 *
 * 在 Next.js 应用中的角色：
 * - 该组件由 `app/admin/role-workbench/[bookId]/page.tsx`（Server Component）渲染并注入首屏数据；
 * - 本组件声明 `'use client'`，属于 Client Component，负责角色资料工作台所有高频交互：
 *   Tab 切换、来源筛选、合并建议处理，以及全局资料视图的刷新。
 *
...
/* ------------------------------------------------
   Component
   ------------------------------------------------ */
export function RoleWorkbenchPanel({
  bookId,
  bookTitle,
  initialDrafts,
  ...
  return (
    <div className="role-workbench-panel flex h-full min-h-0 flex-col gap-4 overflow-hidden">
      {/* 头部区域：展示当前书名与汇总统计，帮助录入人员快速判断待确认规模。 */}
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold text-foreground">{bookTitle}</h2>
          {drafts && (
            <p className="text-sm text-muted-foreground">
              共 {drafts.summary.total} 条待确认资料
