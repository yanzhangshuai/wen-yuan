---
stage: growth
---

# 代码复用思考指南

> **目的**：写新代码前先停下来判断，仓库里是否已经有可复用实现。

---

## 问题本质

**重复代码是“不一致性 bug”的头号来源。**

当你复制粘贴或重写已有逻辑时：
- bug 修复无法同步传播
- 行为会随时间逐步漂移
- 代码库理解成本持续上升

---

## 写新代码前

### 第一步：先搜索

```bash
# 搜索相似函数名
grep -r "functionName" .

# 搜索相似逻辑
grep -r "keyword" .
```

### 第二步：问自己这些问题

| 问题 | 如果是 |
|----------|-----------|
| 是否已有相似函数？ | 复用或扩展它 |
| 这个模式在别处是否已使用？ | 跟随现有模式 |
| 是否应抽成共享 utility？ | 放到正确的共享位置 |
| 我是否正从别的文件复制代码？ | **停止**，先提取为共享实现 |

---

## 常见重复模式

### 模式 1：复制粘贴函数

**反例**：把校验函数复制到另一个文件

**正例**：提取为共享 utilities，按需导入

### 模式 2：高度相似组件

**反例**：新建一个与已有组件 80% 相似的组件

**正例**：通过 props/variants 扩展已有组件

### 模式 3：重复定义常量

**反例**：同一常量在多个文件重复定义

**正例**：单一事实来源（single source of truth），统一导入

代码示例：
```ts
// 反例
export const MAX_RETRY = 3; // 文件 A
export const MAX_RETRY = 5; // 文件 B

// 正例
// constants/retry.ts
export const MAX_RETRY = 3;
// 其他模块：import { MAX_RETRY } from "@/constants/retry";
```

原因：
- 常量多点定义会引发行为漂移，尤其在重试、限流、超时等策略上风险最高。
- 单一来源可显著降低回归风险与全局改动成本。

### 模式 4：重复实现 Hydration 门控逻辑

**反例**：多个组件各自写 `mounted + useEffect` 控制主题首帧。

**正例**：抽成共享 hook，统一输出 `isHydrated` / `selectedTheme`，业务组件只消费。

原因：
- 主题门控逻辑分散后，任一组件漏改都可能触发 hydration mismatch。
- 共享 hook 能让修复一次、全局受益，减少“同类 bug 反复出现”。

### 模式 5：多个弹框各自拉模型列表，导致同类 bug 横向复制

**反例**：姓氏/知识包/泛称谓弹框分别维护 `modelOptions + loading + 默认模型选择`，每个页面复制一套 `fetchModels` 逻辑和过滤条件。

**正例**：抽成统一 `useAdminModels` store hook（模块级缓存 + refresh），页面只消费：
- `models`（支持 `onlyEnabled`）
- `loading/error`
- `defaultModel`

```ts
// 反例：每个页面都写一遍
useEffect(() => {
	fetchModels().then(setModelOptions);
}, []);

// 正例：统一复用
const { models, loading, defaultModel, refresh } = useAdminModels({ onlyEnabled: true });
```

原因：
- 当“模型可选过滤规则”需要调整时，散落实现会出现局部修复、局部漏改。
- 抽到共享 hook 后，选择规则、默认值策略、缓存策略都可以一次修复、全局生效。

### 模式 5.1：共享缓存“只读不重校验”，导致页面长期使用过期数据

**反例**：`useAdminModels` 命中模块级缓存后直接返回；用户在“模型管理”页启用模型后，回到知识库生成弹框仍显示“暂无可用模型”。

**正例**：在关键交互入口（如“打开生成弹框”）调用 `refresh()` 主动重拉；同时在 UI 上区分“加载中 / 加载失败 / 当前为空”三种状态。

```ts
// 反例：缓存命中即返回，不重校验
if (!forced && modelCache) return;

// 正例：在关键入口强制刷新
useEffect(() => {
  if (open) refresh();
}, [open, refresh]);
```

原因：
- 这是“共享状态复用”与“运营配置实时性”之间的冲突，不是单页面 bug。
- 共享缓存若缺少重校验触发点，会把一次空结果放大为跨页面长期错误感知。
- 复用组件必须暴露显式刷新能力，并在业务关键入口建立一致调用约定。

### 模式 6：表单各自写一套下拉实现，导致主题和交互规则漂移

**反例**：有的页面使用原生 `<select>`，有的页面直接拼 Radix Select，另一些页面再各自发明“空值 = \"\"”约定。

**正例**：统一复用 `@/components/ui/select`，并把“空选项”约定沉淀在共享层：
- 主题配色统一走 `bg-background/bg-popover/text-foreground`
- 可清空项统一使用共享 sentinel，而不是 `<SelectItem value="">`
- 业务页面只做 `sentinel ↔ null/""` 映射，不再重复发明实现

原因：
- 下拉是高频基础控件，一旦不复用，最先漂移的就是主题、占位态和空值语义。
- 把可清空规则沉淀到共享组件后，Radix 升级或主题策略调整时可以一次修复、全局生效。

---

## 何时抽象

**适合抽象**：
- 同一逻辑出现 3 次以上
- 逻辑复杂度足以产生 bug
- 多人都可能需要复用

**不适合抽象**：
- 只使用一次
- 非常简单的一行逻辑
- 抽象成本高于重复成本

---

## 批量修改之后

当你在多个文件做了相似改动后：

1. **复查**：是否覆盖了所有实例？
2. **复搜**：再跑一次 grep，看是否有遗漏。
3. **评估**：现在是否应该抽象？
4. **跨页面校验**：复用逻辑若涉及 hydration/SSR，至少在两个入口页面验证首帧一致性。

---

## 易错点：不同机制产出同一结果

**问题**：如果两个不同机制都要生成同一批文件（例如 init 走递归目录拷贝，update 走手写 `files.set()`），目录结构变更（重命名、移动、新增子目录）通常只会被自动机制覆盖，手写机制会静默漂移。

**现象**：init 正常，update 却写错路径或漏文件。

**预防清单**：
- [ ] 迁移目录结构时，搜索所有仍引用旧结构的代码路径
- [ ] 若一路是自动推导（glob/copy），另一路是手写列表，必须同步更新手写列表
- [ ] 增加 regression test，对比两种机制的输出是否一致

---

## 提交前检查

- [ ] 已搜索仓库中现有相似实现
- [ ] 没有本应共享却仍在复制粘贴的逻辑
- [ ] 常量仅在一个来源定义
- [ ] 相似模式保持一致结构
- [ ] 涉及浏览器本地状态（theme/localStorage/media query）时，已复用共享 hydration hook，而非重复写 mounted 门控
- [ ] 多页面都依赖同一模型列表时，已复用统一 store/hook，而非每页各写一套拉取与过滤逻辑
