...
| 场景 | 常见错误 | 对应案例 |
|------|---------|---------|
| API 返回格式与前端期望不符 | 隐式格式假设 | 错误 1 |
| 内部枚举/协议字段直接渲染给用户 | 抽象泄漏 | 错误 3.1 |
| Select 空值用 `""` 导致组件崩溃 | 组件契约泄漏 | 错误 3.2 |
| 用 `dark:` 类但项目是 `data-theme` | 主题机制混淆 | 错误 3.2.1 |
| `<a><button>` 嵌套导致 Hydration 不一致 | DOM 解析与 React 契约不一致 | 错误 4 |
| SSR 首帧用 `localStorage` 状态 → Hydration 警告 | 服务端/客户端状态不一致 | 错误 5 |
| 服务端 Promise 透传给 Client 组件再 `use()` | RSC 数据边界不稳定 | 错误 5.1 |
| 根背景层盖住 admin layout | stacking context 契约缺失 | 错误 5.2 |
| 前端静态评分表与后端实际数据脱节 | 跨层契约断裂 | 错误 6 |
| HTTP 200 与业务 success 混淆 | 传输层/业务层语义混淆 | 错误 7 |
| 新增后台模块但导航/运行时未闭环 | 六层变更传播遗漏 | 错误 8 |
| `clientFetch<WrappedResult>` 导致运行时 `undefined` | 服务层泛型类型违反解包契约 | 错误 9 |

**三步法**：① 画清数据流 `Source → Transform → Store → Display` ② 识别层边界 ③ 定义每个边界的契约（输入/输出/错误类型）
...
### 错误 3.2.1：把主题切换机制误当成组件暗色模式机制，导致主题样式失效

**反例**：项目根层使用 `data-theme="xingkong"` 切换主题，但基础组件仍继续写 `dark:bg-*`、`dark:hover:*`，以为深色主题会自动命中这些类。

**正例**：明确区分“主题驱动机制”和“组件配色语义”。当项目通过 CSS 变量 + `data-theme` 驱动主题时，基础组件只使用 `bg-background`、`bg-popover`、`text-foreground` 这类语义 token，不依赖 `.dark` 选择器。

原因：
- 这是“主题 provider 契约”与“组件样式实现”的跨层问题。
- `.dark` 类没有挂到根节点时，所有 `dark:` 变体都是死代码，排查时却很容易被误判成 token 不生效。
- 一旦基础组件写死具体深色分支，而不是消费语义 token，就会出现“某个主题能看、另一个主题突然发白”的局部漂移。
...
### 错误 24：Prisma Schema 变更后 `as never` mock 类型断言掩盖死代码残留

**反例**：
- 从 `schema.prisma` 删除模型（如 `RelationshipEvent`、`SurnameRule`）后，`prisma generate` 不再产出对应 TypeScript 类型。
- 测试文件中的 mock 使用 `as never` 强制转换 Prisma client 类型，绕过了全部类型检查：

```ts
// 反例：as never 压制了所有类型错误
const hoisted = vi.hoisted(() => ({
  prisma: {
    relationshipEvent: { create: vi.fn() },  // ← 模型已在 schema 中删除
    surnameRule       : { findMany: vi.fn() }
  }
}));
vi.mock("@/server/db/prisma", () => ({
  prisma: hoisted.prisma as never  // ← 编译器无法检测到残留引用
}));
```

- 结果：`tsc --noEmit` 通过，但仓库中存在大量引用已删除模型的死代码（service 函数、API 路由、测试 describe block）。

**正例**：
- 使用 `vi.mocked()` 对已导入的真实 prisma 实例做局部 mock，保留类型检查：
...
- 对必须使用 `vi.hoisted()` 的场景，使用 Prisma client 类型推导而非 `as never`：

```ts
import type { PrismaClient } from "@/generated/prisma/client";
const hoisted = vi.hoisted(() => ({
  prisma: {} as PrismaClient  // ← 仍不安全但至少编译期可报字段不存在的错误
}));
```

- Schema 变更后的清理以「对被删模型名在全仓 grep 无残留引用」为完成标准，不以 `tsc --noEmit` 通过为标准。

**检测信号**：
- `git log -- prisma/schema.prisma` 显示有模型被删除，但 `tsc --noEmit` 仍然通过。
- 用被删模型名（如 `relationshipEvent`、`surnameRule`）grep 全仓仍有命中。
- 测试文件中出现 `as never` 后紧跟 Prisma model 的旧字段名。

**规则**：
- 任何 `as never` 类型断言出现在 Prisma client mock 中，都是一条待修复的技术债。
- Schema 变更（删模型/删字段/改类型）后，必须在全仓 grep 被删标识符，确认无残留后才能真正关闭变更。

原因：
- 这是「Prisma Schema 层」→「编译检查层」→「测试 mock 层」的三层契约断裂。
- `as never` 本来用于简化 mock 编写，但它也彻底破坏了 TypeScript 对 Schema 变更的传播保护能力。
- 当编译器静默时，死代码的唯一发现途径是人工审查，而人工审查在大型重构中极易遗漏。
- 案例：`simplify-relationship-types` 重构中，删除 3 张表后 `tsc` 仍然通过，四轮人工清理才完成：①删表 ②姓名词库全链路清理 ③P0/P1/P2 死代码清理 ④过期注释修复。
...
- [ ] 前端服务层 `clientFetch<T>` 的泛型 `T` 是 API `data` 字段的**直接类型**（如 `Item[]`），而非包含 `{ data, pagination }` 的包裹对象——`pagination` 在信封 `meta.pagination` 中，`clientFetch` 不返回它
