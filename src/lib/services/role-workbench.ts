 ...
 * 文件定位（角色资料工作台客户端服务层）
 * -----------------------------------------------------------------------------
 * 文件路径：`src/lib/services/role-workbench.ts`
 *
 * 本文件属于前端数据访问层，专门封装“角色资料工作台”相关的客户端 HTTP 调用。
 * 在 Next.js 项目中的角色：
 * - 被 `RoleWorkbenchPanel` 等 Client Component 调用；
 * - 向 `app/api/admin/*` Route Handler 发送请求；
 * - 将后端响应映射为前端可直接渲染的 TypeScript 类型。
 *
 * 业务职责：
 * 1) 拉取待确认草稿（人物/关系/传记事件）；
 * 2) 拉取并处理人物合并建议；
 * 3) 触发批量确认入库/拒绝动作。
 *
 * 上游输入：
...
export interface PersonaDraftItem {
  /** 草稿记录 ID（来自 Profile 表），用于单选/批量确认操作。 */
  id          : string;
  ...
  /** 确认状态字符串（通常为 PENDING/VERIFIED/REJECTED）。 */
  status      : string;
}
...
/**
 * fetchDrafts 完整响应结构
 * 包含 summary 计数和三类草稿列表，由角色资料工作台面板整体消费。
 */
...
export async function fetchDrafts(
  ...
  // `bookId` 是角色资料工作台上下文的主键，没有它就无法限定资料范围。
...
export async function fetchChapterEventChapters(bookId: string): Promise<ChapterEventChapterData> {
  return clientFetch<ChapterEventChapterData>(`/api/admin/role-workbench/books/${bookId}/chapter-events`);
}
...
export async function fetchChapterEvents(
  ...
  return clientFetch<ChapterEventItem[]>(`/api/admin/role-workbench/books/${bookId}/chapter-events?${params}`);
}
...
export async function updateChapterEvent(
  ...
  return clientFetch<ChapterEventItem>(`/api/admin/role-workbench/books/${bookId}/chapter-events/${eventId}`, {
    method : "PATCH",
    headers: { "Content-Type": "application/json" },
    body   : JSON.stringify(body)
  });
...
export async function deleteChapterEvent(bookId: string, eventId: string): Promise<void> {
  await clientMutate(`/api/admin/role-workbench/books/${bookId}/chapter-events/${eventId}`, {
    method: "DELETE"
  });
}

export async function markChapterEventsVerified(bookId: string, chapterId: string): Promise<void> {
  await clientMutate(`/api/admin/role-workbench/books/${bookId}/chapter-events/verify`, {
    method : "POST",
    headers: { "Content-Type": "application/json" },
 ...
 *
 * 返回 AI 预填的所有待处理（及历史）合并建议，状态为 PENDING 的显示在角色资料工作台中。
 * 失败时抛出 Error。
 *
 * @param bookId 书籍 UUID
