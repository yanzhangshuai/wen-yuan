 ...
 * -----------------------------------------------------------------------------
 * 文件路径：`app/admin/review/loading.tsx`
 *
 * 框架语义：
 ...
 *
 * 业务作用：
 * - 为审核中心页面提供骨架屏，降低“白屏等待”带来的感知延迟；
 * - 结构上模拟“左侧书籍列表 + 右侧 Tab 与草稿卡片”，让用户提前建立页面预期。
 *
 * 设计原因：
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1 space-y-4">
          <div className="flex gap-2">
            {/* 顶部 Tab 占位：4 个标签对应审核主面板常见 tab 数量。 */}
            {Array.from({ length: 4 }).map((_, i) => (
              <Skeleton key={i} className="h-10 w-24 rounded-md" />
            ))}
          </div>
          {/* 主内容卡片占位：6 行用于覆盖“草稿列表首屏”典型密度。 */}
