...
function parseRuleType(value: string | null): NerLexiconRuleType {
...
function formatModelOption(model: { name: string; provider: string; isDefault: boolean }): string {
...
export function NerRuleGeneratorPanel({
  ...
  useEffect(() => {
    let cancelled = false;
    fetchBookTypes({ active: true }).then((data) => {
      if (!cancelled) setBookTypes(data);
    }).catch((error: unknown) => {
      const message = error instanceof Error ? error.message : String(error);
      toast({ title: "加载书籍类型失败", description: message, variant: "destructive" });
    });
  ...
  useEffect(() => {
    ...
    const stillExists = modelOptions.some((m) => m.id === selectedModelId);
    ...
    <div>
        <div className="grid max-w-3xl gap-4">
            ...
            <Select value={selectedModelId} onValueChange={setSelectedModelId} disabled={modelsLoading || generating}>
              ...
              <SelectContent>
                {modelOptions.length === 0 ? (
                  ...
                  <SelectItem key={model.id} value={model.id}>{formatModelOption(model)}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="grid gap-2">
              <Label>规则类型</Label>
              <Select value={ruleType} onValueChange={(v) => setRuleType(parseRuleType(v))} disabled={generating}>
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 rounded-md bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
            ...
            <span>类型：<span className="font-medium text-foreground">{getRuleTypeLabel(ruleType)}</span></span>
            <span className="text-border">·</span>
            ...
            <div className="space-y-2 rounded-md border p-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-1">
                  ...
                  <pre className="max-h-72 overflow-auto rounded bg-muted p-2.5 text-xs whitespace-pre-wrap">{preview.systemPrompt}</pre>
                </div>
                <div className="space-y-1">
                  <div className="text-xs font-medium">用户提示词</div>
                  <pre className="max-h-72 overflow-auto rounded bg-muted p-2.5 text-xs whitespace-pre-wrap">{preview.userPrompt}</pre>
                </div>
              </div>
            </div>
          ) : null}
