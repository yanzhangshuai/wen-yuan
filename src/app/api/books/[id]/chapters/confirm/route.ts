/**
 * =============================================================================
 * 文件定位（Next.js Route Handler：章节确认接口）
 * -----------------------------------------------------------------------------
 * 文件路径：`src/app/api/books/[id]/chapters/confirm/route.ts`
 *
 * 路由语义：
 * - 映射 `POST /api/books/:id/chapters/confirm`；
 * - `[id]` 为书籍主键，来自动态路由参数；
 * - 运行于服务端，负责把“人工确认章节切分结果”落库。
 *
 * 核心业务流程：
 * 1) 校验章节数组结构（序号、类型、标题、可选正文）；
 * 2) 校验管理员身份后调用 `confirmBookChapters`；
 * 3) 将领域错误（书籍不存在、源文件缺失、payload 违规）映射成稳定 API 错误码。
 *
 * 为什么在接口层做强校验：
 * - 章节确认是后续解析链路的起点数据，若格式脏数据入库会持续污染后续 AI 任务；
 * - 因此这里的校验规则是业务防线，不是编码偏好。
 * =============================================================================
 */
