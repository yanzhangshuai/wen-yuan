...
export async function PATCH(
  ...
  try {
    ...
    const existing = await prisma.historicalFigureEntry.findUnique({ where: { id: parsedParams.data.id } });
    if (!existing) {
      return notFoundJson(PATH, requestId, startedAt, "历史人物不存在");
    }

    const data = await prisma.historicalFigureEntry.update({ where: { id: parsedParams.data.id }, data: parsed.data });
    return okJson({
      path   : PATH, requestId, startedAt,
      code   : "HISTORICAL_FIGURE_UPDATED",
      message: "历史人物更新成功",
      data
    });
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.historicalFigureEntry.delete({ where: { id: parsedParams.data.id } });
    return okJson({
      path   : PATH, requestId, startedAt,
      code   : "HISTORICAL_FIGURE_DELETED",
      message: "历史人物删除成功",
