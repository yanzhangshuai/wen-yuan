...
import { previewGenericTitleGenerationPrompt } from "@/server/modules/knowledge";
import { ERROR_CODES } from "@/types/api";

import { badRequestJson } from "../../../_shared";

const PATH = "/api/admin/knowledge/title-filters/generate/preview-prompt";
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    return okJson({
      ...
      requestId,
      startedAt,
      code   : "ADMIN_GENERIC_TITLE_GENERATE_PROMPT_PREVIEW",
      message: "提示词预览成功",
      data
    });
  } catch (error) {
