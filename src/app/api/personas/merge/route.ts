/**
 * =============================================================================
 * 文件定位（Next.js Route Handler：人物合并接口）
 * -----------------------------------------------------------------------------
 * 文件路径：`src/app/api/personas/merge/route.ts`
 *
 * 该路由提供 `POST /api/personas/merge`，用于处理“同一人物被重复建档”场景：
 * - `sourceId`：被并入的人物；
 * - `targetId`：保留的人物主档。
 *
 * 业务意义：
 * - 人物合并会影响关系边、传记、提及记录等多表数据归属；
 * - 因此接口层必须严格校验并维持明确错误语义，避免误合并造成数据污染。
 *
 * 与下游协作：
 * - 实际合并策略在 `mergePersonas` service 内完成（事务 + 冲突处理）；
 * - 本层只负责请求协议、鉴权与错误映射，确保对前端 contract 稳定。
 * =============================================================================
 */
