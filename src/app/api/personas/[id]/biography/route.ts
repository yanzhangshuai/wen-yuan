/**
 * =============================================================================
 * 文件定位（Next.js Route Handler：人物传记事件创建）
 * -----------------------------------------------------------------------------
 * 文件路径：`src/app/api/personas/[id]/biography/route.ts`
 *
 * 路由语义：
 * - 映射 `POST /api/personas/:id/biography`；
 * - 为指定人物追加一条传记事件记录。
 *
 * 业务职责：
 * 1) 解析人物 ID 与事件内容；
 * 2) 执行管理员鉴权；
 * 3) 调用 `createPersonaBiography` 完成入库并返回前端可直接渲染的数据快照。
 *
 * 为什么单独拆该路由：
 * - 传记事件属于高频编辑对象，与人物基础信息（名称/别名）生命周期不同；
 * - 独立路由便于未来扩展批量导入、模板化录入、审核流等能力。
 * =============================================================================
 */
