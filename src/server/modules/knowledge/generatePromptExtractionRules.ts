...
const generatedPromptExtractionRuleSchema = z.object({
  content   : z.string().trim().min(1),
  confidence: z.number().min(0).max(1).default(0.8)
});

const generatedPromptExtractionRulesSchema = z.array(generatedPromptExtractionRuleSchema);
...
function buildPromptExtractionPrompts(input: {
  ruleType   : PromptExtractionGenerationPreview["ruleType"];
  targetCount: number;
  ...
  const systemPrompt = [
    "你是中文古典文学 Prompt 抽取规则构建助手。",
    "请严格输出 JSON 数组，不要输出任何额外说明、Markdown 或注释。",
    "每个对象必须包含 content、confidence。",
    "content 必须是可直接注入提示词的规则文本，不要输出解释字段。",
    "confidence 低于 0.5 的候选不要输出。"
  ].join("\n");

  const userPrompt = [
    `请为 Prompt 抽取规则类型 ${input.ruleType} 生成 ${input.targetCount} 条候选规则。`,
    input.bookType
      ? `参考题材：${input.bookType.name}（key: ${input.bookType.key}）`
      : "参考题材：未指定，请按通用古典文学场景生成。",
    "目标：补充能直接指导模型抽取实体或关系的简明规则，不要输出剧情摘要或长段解释。",
    "当前已启用规则：",
    existingLines,
    input.additionalInstructions ? `补充要求：${input.additionalInstructions}` : "",
    '输出示例：[{"content":"抽取门派与修行体系","confidence":0.9}]'
  ].filter(Boolean).join("\n");

  return { systemPrompt, userPrompt };
}

export async function previewPromptExtractionGenerationPrompt(input: {
  ruleType               : PromptExtractionGenerationPreview["ruleType"];
  targetCount?           : number;
  bookTypeId?            : string;
  additionalInstructions?: string;
  ...
  const prompts = buildPromptExtractionPrompts({
    ruleType              : input.ruleType,
    targetCount,
...
export async function generatePromptExtractionRules(input: {
  ruleType               : PromptExtractionGenerationPreview["ruleType"];
  targetCount?           : number;
  bookTypeId?            : string;
  additionalInstructions?: string;
  ...
  modelId?               : string;
}): Promise<PromptExtractionGenerationResult> {
  const preview = await previewPromptExtractionGenerationPrompt(input);
  const [generated, existingRules, lastRule] = await Promise.all([
    executeKnowledgeJsonGeneration({
      ...
      schema         : generatedPromptExtractionRulesSchema
    }),
    prisma.promptExtractionRule.findMany({
      where: {
        ruleType  : input.ruleType,
    ...
    prisma.promptExtractionRule.findFirst({
      where: {
        ruleType  : input.ruleType,
        bookTypeId: input.bookTypeId ?? null
      },
  ...
  if (data.length > 0) {
    await prisma.promptExtractionRule.createMany({ data });
  }
