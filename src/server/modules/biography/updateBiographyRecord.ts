 ...
 * 文件定位（服务层：传记事件更新）
 * -----------------------------------------------------------------------------
 * 文件路径：`src/server/modules/biography/updateBiographyRecord.ts`
 *
 * 模块职责：
 * - 对已有传记记录执行部分字段更新；
 * - 维护更新输入校验、空值处理、状态流转一致性。
 *
 * 业务背景：
 * - 传记记录通常在审核阶段反复修订（事件文案、分类、章节归属）；
 * - 采用 PATCH 风格可避免编辑器提交不相关字段导致的覆盖。
 *
 * 维护提醒：
 * - 章节归属变更会影响人物时间线排序与证据回溯，属于高风险变更路径；
 * - 状态字段（如 APPROVED/REJECTED）与审核流程联动，不可随意改默认行为。
 * =============================================================================
 */
