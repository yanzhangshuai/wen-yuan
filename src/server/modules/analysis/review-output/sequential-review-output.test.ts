...
describe("createSequentialReviewOutputAdapter", () => {
  ...
  describe("writeBookReviewOutput", () => {
    it("converts legacy rows into accepted review claims with identity mappings", async () => {
      ...
      expect(tx.entityMention.createMany).toHaveBeenCalledOnce();
      expect(tx.entityMention.createMany).toHaveBeenCalledWith(
        expect.objectContaining({
      ...
      const createdMentionId = tx.entityMention.createMany.mock.calls[0]?.[0].data[0]?.id;
      expect(createdMentionId).toEqual(expect.any(String));
    ...
    it("uses a transaction timeout suitable for full-book review output", async () => {
      const tx = makeTx();
      const prismaClient = makePrisma(tx);
      const adapter = createSequentialReviewOutputAdapter(prismaClient as unknown as PrismaClient);
      ...
      expect(prismaClient.$transaction.mock.calls[0]?.[1].timeout).toBeGreaterThan(5000);
    });

    it("batch creates evidence spans and entity mentions to avoid per-mention transaction queries", async () => {
      const tx = makeTx();
      ...
      expect(tx.evidenceSpan.create).not.toHaveBeenCalled();
      expect(tx.entityMention.create).not.toHaveBeenCalled();
      expect(tx.evidenceSpan.createMany).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.arrayContaining([
            expect.objectContaining({ quotedText: "范进" }),
            expect.objectContaining({ quotedText: "中举" }),
            expect.objectContaining({ quotedText: CHAPTER_CONTENT })
          ])
        })
      );
      expect(tx.entityMention.createMany).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.arrayContaining([
            expect.objectContaining({ surfaceText: "范进" }),
            expect.objectContaining({ surfaceText: "中举" })
          ])
        })
      );
    });
    ...
    it("uses category string as predicate when biography title is blank", async () => {
      const tx = makeTx();
      tx.biographyRecord.findMany.mockResolvedValue([
        {
          id         : BIO_ID_1,
      ...
      expect(tx.eventClaim.createMany).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.arrayContaining([
            expect.objectContaining({ predicate: "SOCIAL" })
          ])
        })
      );
    });
    ...
    it("re-run deletes evidence spans before creating new ones (no orphaned duplicates)", async () => {
      ...
      // 必须在任何 evidenceSpan.createMany 之前被调用，且 where 条件正确作用域。
      const tx = makeTx();
      ...
      tx.evidenceSpan.createMany = vi.fn().mockImplementation(() => {
      ...
      // deleteMany must precede all createMany calls for the same chapter
      const firstCreate  = callOrder.indexOf("createMany");
