...
type TxCallback<T> = (tx: ChapterEventsReviewTransaction) => Promise<T>;
...
export interface ChapterEventsReviewTransaction {
  book: {
...
export interface ChapterEventsReviewPrisma extends ChapterEventsReviewTransaction {
...
async function assertBookExists(tx: ChapterEventsReviewTransaction, bookId: string) {
...
async function assertPersonaInBook(
  tx: ChapterEventsReviewTransaction,
  bookId: string,
  personaId: string
) {
...
export function createChapterEventsReviewService(
  prismaClient: ChapterEventsReviewPrisma = prisma
) {
  ...
  async function markChapterVerified(bookId: string, chapterId: string, verifiedBy?: string) {
    return prismaClient.$transaction(async (tx) => {
      ...
      if (pendingCount > 0) {
        throw new BiographyInputError("当前章节仍有待审核角色事迹，请先处理后再标记已校验");
      }
...
export const {
  ...
  updateEvent,
  deleteEvent
} = createChapterEventsReviewService();
