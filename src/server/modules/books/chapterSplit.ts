...
export function splitRawContentToChapterDrafts(rawContent: string): ChapterSplitDraft[] {
  ...
  // 过滤字数为 0 的空章节（如目录页连续标题行），并重新连续编号。
  const filtered = drafts.filter((item) => item.wordCount > 0);
  ...
  // PRELUDE 从 index 0 开始编号，正文章节（CHAPTER/POSTLUDE）从 1 开始，
  // 使书名页/序章不占用正文序号，第一章始终为 1。
