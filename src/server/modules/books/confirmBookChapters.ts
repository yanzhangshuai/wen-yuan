 ...
 * 文件定位（服务层：书籍章节确认）
 * -----------------------------------------------------------------------------
 * 文件路径：`src/server/modules/books/confirmBookChapters.ts`
 *
 * 层级角色：
 * - 位于服务端领域服务层（非路由层、非组件层）；
 * - 负责把“章节预切分草稿”转换为最终确认章节并持久化。
 *
 * 核心业务问题：
 * - 上传原文后，章节切分可能需要人工修正；
 * - 本模块承接人工确认结果，保证章节编号、标题、正文与字数统计一致。
 *
 * 上下游关系：
 * - 上游：`/api/books/:id/chapters/confirm` Route Handler；
 * - 下游：Prisma（书籍/章节表）、对象存储（原文读取）与章节切分工具函数。
 *
 * 关键维护约束：
 * - 章节确认是后续分析任务的数据基线，任何字段语义变更都可能影响整条解析链路；
