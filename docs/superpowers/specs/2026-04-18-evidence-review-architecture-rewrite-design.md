...
6. `relationTypeKey` 在数据库中保存为字符串键，不使用数据库枚举。
7. 代码层可以内置一组常用关系预设注册表，但代码预设只作为默认种子与 UI 快捷来源，不作为唯一真相源。
8. 标准版及以上应提供 `relation_types` 目录或等价知识对象，统一管理关系 key、显示名、别名映射、作用域和启用状态。
...
本项目对关系类型采用“字符串 key + 预设注册表 + 关系目录”的组合方案，不采用数据库枚举作为 `relationTypeKey` 的底层表示。

明确决策如下：

1. `relationTypeKey` 存库时使用字符串，例如 `teacher_of`、`sworn_brother`、`political_patron_of`。
2. 数据库不为 `relationTypeKey` 建立 enum，因为关系类型是开放集合，允许作品特有关系和用户自定义关系持续增长。
3. 代码层保留一组内置预设关系注册表，用于系统初始化、审核页快捷选择、默认排序和常用关系文案，但代码注册表不是唯一数据源。
4. 系统应提供 `relation_types` 目录表或等价知识对象，作为关系类型的治理层。其职责包括：维护 `key`、默认显示名、同义标签映射、作用域、推荐方向性、是否系统预设、是否启用。
5. 用户在审核页输入自定义关系时，可以直接写入 `relation_claims.relationTypeKey` 与 `relationLabel`；当该关系需要长期复用时，再提升为 `relation_types` 目录项或对应知识条目。
6. 高频自定义关系不要求先建目录、后保存 claim；claim 保存优先，目录治理后置，这样才能保证审核流程顺手。

原因：

1. 数据库 enum 适合闭集字段，不适合可持续增长的开放关系集合，否则每次新增关系都要触发 schema migration。
2. 纯字符串裸存虽然灵活，但长期会导致拼写漂移、统计分裂、同义关系无法收敛，因此必须补上目录治理层。
3. “代码预设 + 数据库字符串”只适合作为过渡实现；完整架构必须增加关系目录层，才能同时满足灵活性、一致性与可审核性。

字段选型原则：

1. 适合用 enum 的字段：`direction`、`relationTypeSource`、`reviewState`、`scopeType`、其他审核状态或来源状态字段。
2. 不适合用 enum 的字段：`relationTypeKey`、`relationLabel`、别名文本、用户可创建的知识键。
3. 只要业务上允许用户扩展、作品扩展、知识回流扩展，该字段就不应设计成数据库 enum。
