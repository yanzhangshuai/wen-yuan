...
- [x] T20: `docs/superpowers/tasks/2026-04-18-evidence-review/20-cutover-read-paths.md`
...
### T20 Completion - 2026-04-24

- Changed files: `docs/superpowers/tasks/2026-04-18-evidence-review/20-cutover-read-paths.md`, `docs/superpowers/plans/2026-04-24-t20-cutover-read-paths-implementation-plan.md`, `src/server/modules/review/evidence-review/persona-detail-read.ts`, `src/server/modules/review/evidence-review/persona-detail-read.test.ts`, `src/app/api/personas/[id]/route.ts`, `src/app/api/personas/[id]/route.test.ts`, `src/app/api/books/[id]/_shared.ts`, `src/app/api/books/[id]/personas/route.ts`, `src/app/api/books/[id]/personas/route.test.ts`, `src/app/api/books/[id]/relationships/route.ts`, `src/app/api/books/[id]/relationships/route.test.ts`, `src/app/admin/books/[id]/page.tsx`, `src/app/admin/books/[id]/page.test.tsx`, `src/app/admin/books/[id]/review-center/page.tsx`, `src/app/admin/books/[id]/review-center/page.test.tsx`, `src/app/admin/review/[bookId]/page.test.tsx`, `src/app/admin/review/[bookId]/relations/page.test.tsx`, `src/app/admin/review/[bookId]/time/page.test.tsx`, `src/app/admin/review/error.tsx`, `src/app/admin/review/error.test.tsx`, `src/app/api/admin/_shared/retired-legacy-review-stack.ts`, `src/app/api/admin/drafts/route.ts`, `src/app/api/admin/drafts/route.test.ts`, `src/app/api/admin/bulk-verify/route.ts`, `src/app/api/admin/bulk-verify/route.test.ts`, `src/app/api/admin/bulk-reject/route.ts`, `src/app/api/admin/bulk-reject/route.test.ts`, `src/app/api/admin/merge-suggestions/**`, `src/app/api/admin/books/[id]/merge-suggestions/**`, `src/app/api/relationships/[id]/route.ts`, `src/app/api/relationships/[id]/route.test.ts`, `src/components/review/index.ts`, and deleted legacy stack files under `src/server/modules/review/`, `src/lib/services/`, and `src/components/review/`.
- Validation commands: `pnpm exec vitest run 'src/server/modules/review/evidence-review/persona-detail-read.test.ts' 'src/app/api/personas/[id]/route.test.ts' 'src/app/api/books/[id]/personas/route.test.ts' 'src/app/api/books/[id]/relationships/route.test.ts' 'src/app/admin/books/[id]/page.test.tsx' 'src/app/admin/books/[id]/review-center/page.test.tsx' 'src/app/api/admin/drafts/route.test.ts' 'src/app/api/admin/bulk-verify/route.test.ts' 'src/app/api/admin/bulk-reject/route.test.ts' 'src/app/api/relationships/[id]/route.test.ts' 'src/app/api/admin/merge-suggestions/route.test.ts' 'src/app/api/admin/merge-suggestions/[id]/accept/route.test.ts' 'src/app/api/admin/merge-suggestions/[id]/reject/route.test.ts' 'src/app/api/admin/merge-suggestions/[id]/defer/route.test.ts' 'src/app/api/admin/books/[id]/merge-suggestions/route.test.ts' 'src/app/api/admin/books/[id]/merge-suggestions/[suggestionId]/accept/route.test.ts' 'src/app/api/admin/books/[id]/merge-suggestions/[suggestionId]/reject/route.test.ts' 'src/app/admin/review/[bookId]/page.test.tsx' 'src/app/admin/review/[bookId]/relations/page.test.tsx' 'src/app/admin/review/[bookId]/time/page.test.tsx' 'src/app/admin/review/error.test.tsx' --coverage=false`, `pnpm exec vitest run 'src/server/modules/review/evidence-review/persona-detail-read.test.ts' 'src/app/api/admin/merge-suggestions/route.test.ts' --coverage=false`, `pnpm type-check`, `pnpm lint`
- Result: T20 finishes the read-path cutover by moving review-adjacent persona detail reads to projection truth, marking remaining legacy book list reads as explicit `TEMP_READ_ONLY_COMPAT`, retiring the old review-center/draft/merge-suggestion/direct-relationship-edit stack behind one stable `410` contract, and adding reviewer-facing no-fallback failure messaging for projection errors.
- Reconciliation note: the cutover references the green T21 baselines at `docs/superpowers/reports/review-regression/rulin-waishi-sample/{summary.md,summary.json}` and `docs/superpowers/reports/review-regression/sanguo-yanyi-sample/{summary.md,summary.json}`; the recorded comparison set remains `儒林外史` persona/chapter-fact/relation/evidence counts `3 / 1 / 2 / 4` and `三国演义` counts `3 / 2 / 3 / 7`.
- Follow-up risks: none at the T20 task boundary; the remaining roadmap task is T22 acceptance and end-to-end verification. Dedicated T20 commit: `eea0481 feat(review): complete t20 read-path cutover and retire legacy review stack`.
- Next task: T22 `docs/superpowers/tasks/2026-04-18-evidence-review/22-e2e-acceptance.md`

### T22 Progress - 2026-04-24

- Changed files: `src/server/modules/review/evidence-review/acceptance/**`, `scripts/review-regression/acceptance/**`, `docs/superpowers/reports/evidence-review-acceptance/**`, `docs/superpowers/tasks/2026-04-18-evidence-review/22-e2e-acceptance.md`, and `docs/superpowers/plans/2026-04-18-evidence-review-superpowers-only-runbook.md`
- Validation commands: `pnpm exec vitest run src/server/modules/review/evidence-review/acceptance/loop-evaluators.test.ts src/server/modules/review/evidence-review/acceptance/runner.test.ts scripts/review-regression/acceptance/run-e2e-acceptance.test.ts --coverage=false --reporter=verbose`, `pnpm type-check`, `pnpm lint`, `pnpm exec tsx scripts/review-regression/acceptance/run-e2e-acceptance.ts --book all`
- Result: the automated T22 acceptance loops and report generation are implemented and reproducible; both books pass the `EVIDENCE`, `REVIEW`, `PROJECTION`, `KNOWLEDGE`, and `REBUILD` loops, and `final-go-no-go.json` reports empty `blockingRisks` plus empty per-book `risks`.
- Follow-up risks: launch remains blocked only by missing human-authored observation files under `docs/superpowers/reports/evidence-review-acceptance/manual-checks/*.json`; until those files exist and are rerun through the CLI, all manual checks stay `PENDING_MANUAL_VERIFICATION` and T22 must remain unchecked in `Task Status`.
- Next task: collect the required manual UI observation JSON files, rerun `pnpm exec tsx scripts/review-regression/acceptance/run-e2e-acceptance.ts --book all`, then mark T22 complete only if the final decision changes from `NO_GO`.
