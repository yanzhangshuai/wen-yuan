...
**测试文件：** `src/server/modules/books/startBookAnalysis.test.ts`

| 测试用例 | 期望状态 | 验收条件 |
|---|---|---|
| Admin 启动全书解析 | 200 | `analysis_jobs` 创建，`status=QUEUED` |
| 书籍 `status` 更新为 `PROCESSING` | — | `books.status = PROCESSING` |
| 指定 `scope=CHAPTER_RANGE` + 合法范围 | 200 | `chapter_start/end` 写入任务 |
...
| Viewer 调用 | 403 | 权限拒绝 |
| 携带 `overrideStrategy=DRAFT_ONLY` | 200 | 参数写入任务记录 |
| 携带 `overrideStrategy=ALL_DRAFTS` | 200 | 参数写入任务记录 |
| 携带 `keepHistory=true` | 200 | 参数写入任务记录 |
| `FULL_BOOK` 不携带范围参数 | 200 | `chapter_start/end` 为 null |
...
| 测试用例 | 验收条件 |
|---|---|
| 返回 `status/progress/stage` 字段 | 结构符合 api-contracts §6.1 |
...
6. POST /api/books/{bookId}/analyze → 启动解析，获取 jobId
