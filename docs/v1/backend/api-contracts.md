...
端点：`POST /api/books/:id/analyze`

- 成功状态码：`202 Accepted`
- 成功业务码：`BOOK_ANALYSIS_STARTED`
- 请求体字段（均可选）：
  - `aiModelId`：UUID；为空时回落为书籍当前绑定模型。
  - `scope`：`FULL_BOOK | CHAPTER_RANGE`，默认 `FULL_BOOK`。
  - `chapterStart/chapterEnd`：仅 `CHAPTER_RANGE` 必填，且需为正整数且 `start <= end`。
  - `overrideStrategy`：`DRAFT_ONLY | ALL_DRAFTS`，默认 `DRAFT_ONLY`。
  - `keepHistory`：是否保留历史任务记录，默认 `false`。
- 行为说明：
  - 路由会先创建任务，再异步触发执行器 `runAnalysisJobById(jobId)`。
  - 路由不阻塞等待执行完成；执行阶段状态由 `analysis_jobs` 与 `/api/books/:id/status` 提供。
- 典型错误映射：
  - 书籍不存在：`404 COMMON_NOT_FOUND`
  - 模型不可用/范围非法：`400 COMMON_BAD_REQUEST`
  - 其他未预期异常：`500 COMMON_INTERNAL_ERROR`
