...
1. 安装依赖：`pnpm install`
2. 检查 PostgreSQL 就绪：`pg_isready -d "$DATABASE_URL"`
3. 生成 Prisma Client：`pnpm prisma:generate`
4. 执行迁移：`pnpm prisma:migrate`
5. 初始化种子：`pnpm prisma:seed`
6. 启动开发：`pnpm dev`

说明：当前 `dev` 脚本端口为 `3060`（见 `package.json`）。
补充：若本机没有 `pg_isready`，可改用 `psql "$DATABASE_URL" -c "select 1;"` 验证连接。
...
- 类型与风格：`pnpm lint`
- 单测：`pnpm test:unit`
- Prisma 状态：`pnpm exec prisma migrate status`
- PostgreSQL 连通性：`pg_isready -d "$DATABASE_URL"`
- Neo4j 可用性（可选）：`cypher-shell -a "$NEO4J_URI" -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" "RETURN 1 AS ok;"`
...
2. 若需启用 Neo4j，先补齐 `NEO4J_URI`、`NEO4J_USER`、`NEO4J_PASSWORD`。
3. 先做网络连通性检查（按你的 URI 替换主机和端口）：`nc -vz <neo4j-host> <neo4j-port>`
4. 再做查询可用性检查：`cypher-shell -a "$NEO4J_URI" -u "$NEO4J_USER" -p "$NEO4J_PASSWORD" "RETURN 1 AS ok;"`
5. 若机器未安装 `cypher-shell`，可在 Neo4j 容器内执行同样命令，或临时使用系统回退路径继续开发。
