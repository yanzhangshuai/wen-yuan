...
| `src/app/api/books/[id]/analyze/route.ts` | 启动书籍解析任务（支持范围、模型、重跑策略），创建后异步调度任务执行器。 |
...
| 文件 | 主要功能 |
| --- | --- |
| `src/server/modules/analysis/jobs/runAnalysisJob.ts` | 解析任务执行器：任务抢占、章节循环执行、进度推进、失败回写与任务恢复。 |
