...
> **已废弃：v7 起由 `15-agent-architecture-v7.md`（逐章提取 + 实体验收闸）取代。**
> v7 推翻的是"分片提取 + 猜章节"的执行形态：分片让事实↔章节归属不确定，
> 衍生出的 chapterNo 输出与 evidence 反查在 BIOGRAPHY 上实测失败（仅 24/91）；
> v7 改为逐章提取（章节归属天然正确），并新增实体验收闸/dropped 软删/personaCount 口径修正。
> 核心运行组件（身份折叠/归并/聚合/审核）原样复用。
