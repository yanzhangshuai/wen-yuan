...
- **models/** — AI 模型配置 CRUD、导入导出、连通性测试与协议化客户端分发
...
所有 AI Prompt 统一从数据库读取（`prompt_templates` + `prompt_template_versions` 表），不存在硬编码 fallback。DB 无模板时直接抛错。核心函数 `resolvePromptTemplate({ slug, bookTypeId?, replacements })` 位于 `src/server/modules/knowledge/prompt-templates.ts`。种子数据由 `prompt-template-baselines.ts` 维护，通过 `prisma/seed.ts` 入库。
